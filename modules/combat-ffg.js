        ...
        let promise = new Promise(async function (resolve, reject) {
            ...
            if (Array.isArray(ids) && ids.length > 1) {
                ...
                const c = initiative.getCombatantByToken(
                        ...
                        .tokenId);
                var data = c.actor.system;
                whosInitiative = c.actor.name;

                /*PATCH VEHICLE COMBAT INITIATIVE*/
                if (c.actor.type === 'vehicle') {
                    const crew = c.actor.getFlag('starwarsffg', 'crew');
                    if (crew) {
                        for (let i = 0; i < crew.length; i++) {
                            if (crew[i].role.includes('Pilot')) {
                                data = game.actors.get(crew[i].actor_id).system;
                ...
                initSkills.forEach((skill) => {
                    if (dicePools.find((p) => p.name === skill))
                        return;
                    ...
                    one: {
                        ...
                        callback: async() => {
                            ...
                            const [updates, messages] = await ids.reduce(
                                    ...
                                    // Get Combatant data
                                    var c = initiative.getCombatantByToken(
                                            initiative.combatants.map(combatant => combatant)
                                    ...
                                    var systemCombattant = c.actor.system;
                                    /*PATCH VEHICLE COMBAT INITIATIVE*/
                                    if (c.actor.type === 'vehicle') {
                                        const crew = c.actor.getFlag('starwarsffg', 'crew');
                                        if (crew) {
                                            for (let i = 0; i < crew.length; i++) {
                                                if (crew[i].role.includes('Pilot')) {
                                                    systemCombattant = game.actors.get(crew[i].actor_id).system;
                                                }
                                            }
                                        } else {
                                            return resolve(results);;
                                    ...
                                    /*EN PATH*/
                                    // Detemine Formula
                                    let pool = _buildInitiativePool(systemCombattant, baseFormulaType);
                                    ...
                                    let roll = new RollFFG(pool.renderDiceExpression(), rollData, {
                                    ...
                                    let messageData = mergeObject({
                                        ...
                                        flavor: `${c.token.name} ${game.i18n.localize("SWFFG.InitiativeRoll")} (${game.i18n.localize(` SWFFG.SkillsName$ {
                                            baseFormulaType.replace(/[: ]/g, "")
                                        }
`)})`,
                                        flags: {
