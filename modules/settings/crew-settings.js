export default class CrewSettings extends FormApplication {
    ...
    getData(options) {
        ...
        for (let setting of gs.settings.values()) {
            ...
            if (!setting.key.includes("arrayCrewRoles") || (!canConfigure && setting.scope !== "client"))
                continue;
            ...
            // Classify setting
            if (s.namespace === game.system.id && s.key.includes("arrayCrewRoles"))
        ...
        Object.keys(data.skills).forEach(key => {
            data.skills[key].value = game.i18n.localize(data.skills[key].label);
        });
